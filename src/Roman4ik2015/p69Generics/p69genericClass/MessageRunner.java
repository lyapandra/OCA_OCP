package Roman4ik2015.p69Generics.p69genericClass;

/**
 * Created by a on 13.04.2018.
 * @author Roman4ik
 */
public class MessageRunner {
    public static void main(String[ ] args) {
// параметризация типом Integer
        Message<Integer> ob1 = new Message<>();
        ob1.setValue(1); // возможен только тип Integer для метода setValue
        int v1 = ob1.getValue();
        Integer v11 = ob1.getValue();
        System.out.println("v1 = " + v1);
        System.out.println("v11 = " + v11);
        System.out.println("ob1 = " + ob1);
//        ob1.setValue(null);
// параметризация типом String
        Message<String> ob2 = new Message<>("Java");
        String v2 = ob2.getValue();
        System.out.println("v2 = " + v2);
        System.out.println("ob2.getValue() = " + ob2.getValue());
        System.out.println("ob2 = " + ob2);
//        ob2.setValue(null);
// ob1 = ob2; // ошибка компиляции – параметризация нековариантна
// параметризация по умолчанию – Object
        Message ob3 = new Message(); // warning – raw type
        ob3 = ob1; // нет ошибки компиляции – нет параметризации
        System.out.println("ob3.getValue() = " + ob3.getValue());
        ob3.setValue(new Byte((byte)1));

        ob3=ob2;
        System.out.println("ob3 = " + ob3);
        ob2=ob3;
        ob1= ob3;
        System.out.println("ob1.getClass().getTypeName() = " + ob1.getClass().getTypeName());
//        ob1 = ob2; ERROR

        System.out.println("ob1<Integer> = " + ob1); // !!! ob1<Integer> = java.lang.String :Java

        ob3.setValue("Java SE 7");
        System.out.println("ob3 = " + ob3);/* выводится тип объекта,
а не тип параметризации */
        ob3.setValue(71);
        System.out.println("ob3 = " + ob3);
        ob3.setValue(null);
        /*Переменные такого типа могут вызывать только методы
        класса Object.*/
        ob3.getValue();
        ob1.getValue();
    }
}
